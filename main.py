# ================================================================================
# Essa estrutura é igual (básica) para todos os aplicativos
# ================================================================================
...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(f"https://aplicativovendashash-edeaa-default-rtdb.firebaseio.com/"
    ...
    def mudar_foto_perfil(self, foto, *kwargs):
        foto_perfil = self.root.ids['foto_perfil']
        foto_perfil.source = f'icones/fotos_perfil/{foto}'

        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativovendashash-edeaa-default-rtdb.firebaseio.com/"
                                    f"{self.local_id}.json?auth={self.id_token}", data=str(info))
        self.mudar_tela("AjustesPage")

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-edeaa-default-rtdb.firebaseio.com/.json' \
               f'?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f"https://aplicativovendashash-edeaa-default-rtdb.firebaseio.com/"
    ...
    def selecionar_cliente(self, foto_cliente, *args):
        ...
        # pintar de branco todas os nomes de cliente
        pagina_adicionarvendas = self.root.ids["adicionarvendaspage"]
        lista_clientes = pagina_adicionarvendas.ids["lista_clientes"]

        for item in list(lista_clientes.children):
            item.color = (1, 1, 1, 1)
            # pintar de azul o cliente do item selecionado
    ...
    def selecionar_produto(self, foto_produto, *args):
        ...
        # pintar de branco todas os nomes de produtos
        pagina_adicionarvendas = self.root.ids["adicionarvendaspage"]
        lista_produtos = pagina_adicionarvendas.ids["lista_produtos"]

        for item in list(lista_produtos.children):
            item.color = (1, 1, 1, 1)
            # pintar de azul o produto do item selecionado
            try:
                texto = item.text
                texto = texto.lower() + ".png"
    ...
    def selecionar_unidade(self, id_label, *args):
        ...
        # pintar de branco todas as unidades
    ...
    def adicionar_venda(self):
        ...
        # Dado que foi tudo preenchido, executar o código para adicionar venda
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) \
            ...
            requests.post(f"https://aplicativovendashash-edeaa-default-rtdb.firebaseio.com/"
                          f"{self.local_id}/vendas.json?auth={self.id_token}", data=info)
            ...
            requests.patch(f"https://aplicativovendashash-edeaa-default-rtdb.firebaseio.com/{self.local_id}.json"
    ...
    def carregar_todas_vendas(self):
        ...
        self.limpar_banner(lista_vendas)

        # preencher a pagina todasvendaspage
        # pegar informações da empresa
        link = f'https://aplicativovendashash-edeaa-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"'
    ...
    def carregar_vendas_vendedor(self, dic_info_vendedor, *kwargs):
    ...
    def limpar_banner(self, lista_limpar):
        # ==============================
        # lista_vendas.clear_widgets()
        # ==============================
        for item in list(lista_limpar.children):
            lista_limpar.remove_widget(item)
